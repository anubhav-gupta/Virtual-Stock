...
class VirtualTrader
{
	...
	{
		$url = "http://finance.google.com/finance/info?client=ig&q=SENSEX:" . $stockcode;
		print $url;
		 ...
		 */
		//<new>
		print $stockinfo['code'] = $obj->{'t'}; // Stock code name (ex: GOOG)
		print "\n".$stockinfo['name'] = $obj->{'t'}; // Stock Company Name (ex: Google Inc.)
		print "\n".$stockinfo['price'] = floatval($obj->{'l_fix'}); // Stock price
		print "\n".$stockinfo['diff'] = floatval($obj->{'c'}); // Stock Difference
		print "\n".$stockinfo['diff_perc'] = floatval($obj->{'cp'}); // Stock difference in percent
		//</new>
	...
	function UpdateStockDB()
	{
		...
		else 
		{
			while($query->fetch())
			{
				...
				$query2 = $this->mysqli->prepare("UPDATE stocks SET price=?, diff=?, diff_perc=? WHERE code=?");
				echo $stockinfo['price'], $stockinfo['diff'], $stockinfo['diff_perc'], $stockinfo['code'];
				//echo $query2->bind_param($stockinfo['price'], $stockinfo['diff'], $stockinfo['diff_perc'],$stockinfo['code']);
				echo $query2->bind_param("ddds",$stockinfo['price'], $stockinfo['diff'], $stockinfo['diff_perc'],$stockinfo['code']);
				$query2->execute();
				$query2->close();
	...
	function BuyShare($stockcode, $quantity, $username)
	{
		...
		{
			//go to temp
			$query = $this->mysqli->prepare("INSERT INTO tempque (code, username, quantity, p_price) VALUES (?, ?, ?, ?)");
			$query->bind_param("ssid", $stockcode, $username, $quantity, $stockinfo['price']);
			if($query->execute());
			write_mysql_log("Something happened!");
			...
			else
			{
				...
				else
				{
					...
					if (($currentTime > $resttimefrom) || ($currentTime < $resttimeto) )
					{
						...
						$query = $this->mysqli->prepare("INSERT INTO tempque (code, username, quantity, p_price) VALUES (?, ?, ?, ?)");
						$check=1;
						$query->bind_param("ssid", $stockcode, $username, $quantity,$check);
						if(!$query->execute())
						{

							$this->successmsg[] = sprintf($lang[$loc]['virtualtrader']['sellshare_success'], $quantity, $query->error, $totalprice);
